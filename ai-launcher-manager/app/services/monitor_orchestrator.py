...
class MonitorOrchestrator:
    """Translates classification results into job state transitions.

    Extracted from ``SessionMonitor`` so that the monitor is responsible
    only for *when* to classify a job while this class owns *what happens
    next*.  Every method that previously mutated job state and persisted
    it now lives here.
    """
    ...
    def __init__(
        ...
        self.logger = get_logger("ai_launcher_manager.orchestrator")

    # ------------------------------------------------------------------
    # Public entry points
    # ------------------------------------------------------------------

    async def handle_cancellation(self, job: JobRecord) -> JobRecord:
        """Process a job whose cancellation was requested."""
        ...
        job.transition(
            JobState.CANCELLED,
            "Job cancelled by operator and host tmux session terminated",
            "monitor",
        )
    ...
    async def apply_classification(
        ...
        """Apply a classification result to a job, executing the
        appropriate state transition and side-effects."""
    ...
    async def handle_missing_window(self, job: JobRecord) -> JobRecord:
        """Handle the case where a job's tmux window has disappeared."""
        if job.can_retry:
